...
namespace MshErp.Infrastructure
{
    ...
    {
        private static readonly string Secret =ConfigHelper.SystemSecrect;
        private static readonly int ExpireMinutes = int.Parse(ConfigHelper.TokenExpireMinutes);
        private const string Issuer = "妙生活";
        ...
        public static IDictionary<string, dynamic> Decode(string token)
        {
            try
            {
                ...
                var handler = new JwtSecurityTokenHandler();
                var principle = handler.ValidateToken(token, validateParam, out SecurityToken validToken);
